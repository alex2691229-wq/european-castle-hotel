...
describe("Overdue Payment Warning", () => {
  ...
  it.skip("should return false for paid bookings", () => {
  ...
  it.skip("should return false for completed bookings", () => {
    mockBooking.status = "completed";
    mockBooking.createdAt = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000); // 10 days ago
    expect(isOverduePayment(mockBooking)).toBe(false);
  });

  it.skip("should return false for cancelled bookings", () => {
    mockBooking.status = "cancelled";
    mockBooking.createdAt = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000); // 10 days ago
    expect(isOverduePayment(mockBooking)).toBe(false);
  });

  it.skip("should return false for pending bookings created less than 3 days ago", () => {
    mockBooking.status = "pending";
    mockBooking.createdAt = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000); // 2 days ago
    expect(isOverduePayment(mockBooking)).toBe(false);
  });

  it.skip("should return false for confirmed bookings created exactly 3 days ago", () => {
    mockBooking.status = "confirmed";
    mockBooking.createdAt = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000); // 3 days ago
    expect(isOverduePayment(mockBooking)).toBe(false);
  });

  it.skip("should return true for pending bookings created more than 3 days ago", () => {
    mockBooking.status = "pending";
    mockBooking.createdAt = new Date(Date.now() - 4 * 24 * 60 * 60 * 1000); // 4 days ago
    expect(isOverduePayment(mockBooking)).toBe(true);
  });

  it.skip("should return true for confirmed bookings created more than 3 days ago", () => {
    mockBooking.status = "confirmed";
    mockBooking.createdAt = new Date(Date.now() - 5 * 24 * 60 * 60 * 1000); // 5 days ago
    expect(isOverduePayment(mockBooking)).toBe(true);
  });

  it.skip("should return true for paid_pending bookings created more than 3 days ago", () => {
    mockBooking.status = "paid_pending";
    mockBooking.createdAt = new Date(Date.now() - 6 * 24 * 60 * 60 * 1000); // 6 days ago
    expect(isOverduePayment(mockBooking)).toBe(true);
  });

  it.skip("should return true for pending bookings created 10 days ago", () => {
    mockBooking.status = "pending";
    mockBooking.createdAt = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000); // 10 days ago
    expect(isOverduePayment(mockBooking)).toBe(true);
  });

  it.skip("should correctly handle edge case: 3 days and 1 hour ago", () => {
    mockBooking.status = "pending";
  ...
  it.skip("should correctly handle edge case: 2 days and 23 hours ago", () => {
    mockBooking.status = "pending";
    mockBooking.createdAt = new Date(Date.now() - (2 * 24 * 60 * 60 * 1000 + 23 * 60 * 60 * 1000)); // 2 days 23 hours ago
    expect(isOverduePayment(mockBooking)).toBe(false);
  });

  it.skip("should return false for paid_pending bookings created 1 day ago", () => {
    mockBooking.status = "paid_pending";
    mockBooking.createdAt = new Date(Date.now() - 1 * 24 * 60 * 60 * 1000); // 1 day ago
    expect(isOverduePayment(mockBooking)).toBe(false);
  });

  it.skip("should return true for multiple statuses when overdue", () => {
